...
app.post("/ferramentas", async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(400)
      .json({ erro: "Erro ao cadastrar ferramenta. Esse código já existe?" });
  }
});
...
app.post("/emprestar", async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const mecanico = await prisma.usuario.findUnique({
      where: { sigla: sigla_mecanico },
    });
    const responsavel = await prisma.usuario.findUnique({
      where: { sigla: sigla_responsavel },
    });
    ...
    if (!mecanico || !responsavel || !ferramenta) {
      return res
        .status(404)
        .json({ erro: "Dados inválidos. Verifique as siglas ou o código." });
    }
    ...
    if (ferramenta.status !== "DISPONIVEL") {
      return res
        .status(400)
        .json({
          erro: "Ação bloqueada: Esta ferramenta já está em uso ou em manutenção!",
        });
    }
  ...
  try {
    ...
    if (ferramenta.status !== "EM_USO") {
      return res
        .status(400)
        .json({
          erro: "Ação bloqueada: Esta ferramenta já está disponível no painel.",
        });
    }
    ...
    if (!movimentacaoAberta) {
      return res
        .status(400)
        .json({
          erro: "Nenhuma ficha de empréstimo aberta para esta ferramenta.",
        });
    }

    // 2. Procura as pessoas que estão devolvendo no banco de dados
    const mecanicoDev = await prisma.usuario.findUnique({
      where: { sigla: sigla_mecanico },
    });
    const supervisorDev = await prisma.usuario.findUnique({
      where: { sigla: sigla_supervisor },
    });
    ...
    if (!mecanicoDev || !supervisorDev) {
      return res
        .status(404)
        .json({
          erro: "Sigla de devolução inválida. Verifique se estão corretas.",
        });
    }
    ...
    res
      .status(200)
      .json({
        mensagem: "Ferramenta devolvida com sucesso! Histórico atualizado.",
      });
  } catch (error) {
